...
-module(resource_google_authorize).
-author("Maxim Treskin <zerthurd@gmail.com>").
...
moved_temporarily(ReqData, Context) ->
    ...
    {AppId, _AppSecret} = mod_google:get_appid_secret(Context1),
    Page = get_page(Context1),
    RedirectUrl = lists:flatten(
                        z_context:abs_url(
                            lists:flatten(z_dispatcher:url_for(google_redirect, [{p,Page}], Context1)),
                            Context1)),
    Location = "https://graph.google.com/oauth/authorize?client_id="
